...
class Categories extends Widget
{
    ...
    {
        $tree = Tree::findOne(Yii::$app->variable->get('kalibao.backend',
            'catalog_tree_id'));
        ...
        $this->categoryUrl = '/' . CmsPageService::getSlugById(47,
                Yii::$app->language) . '/';
    }
...
<div class="panel panel-default">
 <div class="panel-heading">Menu du shop</div>
 <div class="panel-body">
  <ul><li>
   <a href="' . Yii::$app->getUrlManager()->createUrl([$this->categoryUrl]) . '">
   Accueil</a>' . $this->htmlTree($this->treeData) . '
  </li></ul>
 </div>
</div>';
    }
    ...
    private function htmlTree($data)
    {
        ...
        foreach ($data as $d) {
            $id = explode('-', $d['id'])[1];
            $html .= '<li><a href="' . Yii::$app->getUrlManager()->createUrl([
                    $this->categoryUrl . $this->treePaths[$id],
                    'cat' => $id
                ]) . '">' . $d['text'] . '</a>';
