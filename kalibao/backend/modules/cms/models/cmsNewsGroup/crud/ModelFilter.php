...
namespace kalibao\backend\modules\cms\models\cmsNewsGroup\crud;
...
/**
 * This is the model filter class for controller "CmsNewsGroup".
 *
 * @property integer $id
 ...
 *
 * @package kalibao\backend\modules\cms\models\cmsNewsGroup\crud
 * @version 1.0
 * @author Kevin Walter <walkev13@gmail.com>
 */
class ModelFilter extends CmsNewsGroup implements ModelFilterInterface
{
    ...
    public function scenarios()
    {
        return [
            self::SCENARIO_DEFAULT => [
                'id', 'cms_news_group_i18n_title', 'created_at_start', 'created_at_end', 'updated_at_start', 'updated_at_end'
            ]
        ];
    }
    ...
    public function rules()
    {
        return [
            ...
            [['cms_news_group_i18n_title'], 'string', 'max' => 255]
        ];
    }
    ...
    public function attributeLabels()
    {
        ...
        $attributeLabels['cms_news_group_i18n_title'] = Yii::t('kalibao','model:title');
    ...
    public function search($requestParams, $language = null, $pageSize = 10)
    {
        ...
        $query->andFilterWhere(['>=', 'cms_news_group.created_at', $this->created_at_start]);
        if ($this->created_at_end != '') {
            $query->andWhere([
                '<=',
                'cms_news_group.created_at',
                (new \DateTime($this->created_at_end))->modify('+1 day')->format('Y-m-d')
            ]);
        }
        $query->andFilterWhere(['>=', 'cms_news_group.updated_at', $this->updated_at_start]);
        if ($this->updated_at_end != '') {
            $query->andWhere([
                '<=',
                'cms_news_group.updated_at',
                (new \DateTime($this->updated_at_end))->modify('+1 day')->format('Y-m-d')
            ]);
        }

        $query
            ->andFilterWhere(['cms_news_group.id' => $this->id])
